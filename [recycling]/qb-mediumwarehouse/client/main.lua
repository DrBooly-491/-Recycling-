...
local entranceTargetID = 'entranceMediumTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitMediumITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse Medium
local function RegisterEntranceMediumTarget()
  local coords = vector3(Config.MediumOutsideLocation.x, Config.MediumOutsideLocation.y, Config.MediumOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.MediumOutsideLocation.z - 1.0,
      maxZ = Config.MediumOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-medium:client:target:enterLocation',
          label = Lang:t("text.enter_medium_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_enter_medium_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  local coords = vector3(Config.MediumInsideLocation.x, Config.MediumInsideLocation.y, Config.MediumInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.MediumInsideLocation.z - 1.0,
      maxZ = Config.MediumInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-medium:client:target:exitLocation',
          label = Lang:t("text.exit_medium_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_exit_medium_warehouse"), 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function RegisterDutyTarget()
  local coords = vector3(Config.MediumDutyLocation.x, Config.MediumDutyLocation.y, Config.MediumDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.MediumDutyLocation.z - 2.0,
      maxZ = Config.MediumDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-medium:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.MediumDropLocation.x, Config.MediumDropLocation.y, Config.MediumDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.MediumDropLocation.z - 2.0,
      maxZ = Config.MediumDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-medium:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.MediumPickupLocations[math.random(1, #Config.MediumPickupLocations)]
...
local function DropPackage()
  ...
  carryPackage = nil
	...
	--Warehouse Medium Blip
local function SetLocationBlip()
  local RecycleBlip = AddBlipForCoord(Config.MediumOutsideLocation.x, Config.MediumOutsideLocation.y, Config.MediumOutsideLocation.z)
...
local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.MediumPickupLocations) do
    local model = GetHashKey(Config.MediumWarehouseObjects[math.random(1, #Config.MediumWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.MediumInsideLocation.x, Config.MediumInsideLocation.y, Config.MediumInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.MediumOutsideLocation.x, Config.MediumOutsideLocation.y, Config.MediumOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-medium:client:target:pickupPackage',
...
local function DrawPackageLocationBlip()
  ...
  DrawMarker(22, packageCoords.x, packageCoords.y, packageCoords.z + 3, 0, 0, 0, 180.0, 0, 0, 0.5, 0.5, 0.5, 255, 0, 0, 100, false, false, 2, true, nil, nil, false)
end

-- Events

RegisterNetEvent('qb-recyclejob-medium:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-medium:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-medium:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-medium:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-medium:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-mediumwarehouse:server:getItemTwo')
      GetRandomPackage()
  end)
end)
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-medium:client:target:MediumenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-medium:client:target:MediumexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-medium:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-medium:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-medium:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end

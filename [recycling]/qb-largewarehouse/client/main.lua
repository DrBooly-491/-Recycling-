...
local entranceTargetID = 'entranceLargeTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitLargeITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse Large
local function RegisterEntranceLargeTarget()
  local coords = vector3(Config.LargeOutsideLocation.x, Config.LargeOutsideLocation.y, Config.LargeOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.LargeOutsideLocation.z - 1.0,
      maxZ = Config.LargeOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Large:client:target:enterLocation',
          label = Lang:t("text.enter_large_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_enter_large_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  local coords = vector3(Config.LargeInsideLocation.x, Config.LargeInsideLocation.y, Config.LargeInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.LargeInsideLocation.z - 1.0,
      maxZ = Config.LargeInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Large:client:target:exitLocation',
          label = Lang:t("text.exit_large_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_exit_large_warehouse"), 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function RegisterDutyTarget()
  local coords = vector3(Config.LargeDutyLocation.x, Config.LargeDutyLocation.y, Config.LargeDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.LargeDutyLocation.z - 2.0,
      maxZ = Config.LargeDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Large:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.LargeDropLocation.x, Config.LargeDropLocation.y, Config.LargeDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.LargeDropLocation.z - 2.0,
      maxZ = Config.LargeDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Large:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.LargePickupLocations[math.random(1, #Config.LargePickupLocations)]
...
local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.LargePickupLocations) do
    local model = GetHashKey(Config.LargeWarehouseObjects[math.random(1, #Config.LargeWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.LargeInsideLocation.x, Config.LargeInsideLocation.y, Config.LargeInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.LargeOutsideLocation.x, Config.LargeOutsideLocation.y, Config.LargeOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Large:client:target:pickupPackage',
...
RegisterNetEvent('qb-recyclejob-Large:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-Large:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-Large:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-Large:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-Large:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-largewarehouse:server:getItemThree')
      GetRandomPackage()
  end)
end)
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-Large:client:target:LargeenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-Large:client:target:LargeexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-Large:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-Large:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-Large:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end

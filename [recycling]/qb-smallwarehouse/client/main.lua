...
local entranceTargetID = 'entranceSmallTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitSmallITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse Small
local function RegisterEntranceSmallTarget()
  local coords = vector3(Config.SmallOutsideLocation.x, Config.SmallOutsideLocation.y, Config.SmallOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.SmallOutsideLocation.z - 1.0,
      maxZ = Config.SmallOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-small:client:target:enterLocation',
          label = Lang:t("text.enter_small_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_enter_small_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  local coords = vector3(Config.SmallInsideLocation.x, Config.SmallInsideLocation.y, Config.SmallInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.SmallInsideLocation.z - 1.0,
      maxZ = Config.SmallInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-small:client:target:exitLocation',
          label = Lang:t("text.exit_small_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_exit_small_warehouse"), 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function RegisterDutyTarget()
  local coords = vector3(Config.SmallDutyLocation.x, Config.SmallDutyLocation.y, Config.SmallDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.SmallDutyLocation.z - 2.0,
      maxZ = Config.SmallDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-small:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.SmallDropLocation.x, Config.SmallDropLocation.y, Config.SmallDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.SmallDropLocation.z - 2.0,
      maxZ = Config.SmallDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-small:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.SmallPickupLocations[math.random(1, #Config.SmallPickupLocations)]
...
local function DropPackage()
  ...
  carryPackage = nil
	...
	--Warehouse Small Blip
local function SetLocationBlip()
  local RecycleBlip = AddBlipForCoord(Config.SmallOutsideLocation.x, Config.SmallOutsideLocation.y, Config.SmallOutsideLocation.z)
...
local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.SmallPickupLocations) do
    local model = GetHashKey(Config.SmallWarehouseObjects[math.random(1, #Config.SmallWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.SmallInsideLocation.x, Config.SmallInsideLocation.y, Config.SmallInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.SmallOutsideLocation.x, Config.SmallOutsideLocation.y, Config.SmallOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-small:client:target:pickupPackage',
...
local function DrawPackageLocationBlip()
  ...
  DrawMarker(22, packageCoords.x, packageCoords.y, packageCoords.z + 2, 0, 0, 0, 180.0, 0, 0, 0.5, 0.5, 0.5, 255, 0, 0, 100, false, false, 2, true, nil, nil, false)
end

-- Events

RegisterNetEvent('qb-recyclejob-small:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-small:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-small:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-small:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-small:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-smallwarehouse:server:getItemOne')
      GetRandomPackage()
  end)
end)
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-small:client:target:smallenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-small:client:target:smallexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-small:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-small:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-small:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end

...
local entranceTargetID = 'entranceCarTarget'
local isInsideEntranceZone = false
local entranceZone = nil

local exitTargetID = 'exitCarITarget'
...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
	...
	--Warehouse Car
local function RegisterEntranceCarTarget()
  local coords = vector3(Config.CarOutsideLocation.x, Config.CarOutsideLocation.y, Config.CarOutsideLocation.z)

  if Config.UseTarget then
    entranceZone = exports['qb-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      name = entranceTargetID,
      heading = 44.0,
      minZ = Config.CarOutsideLocation.z - 1.0,
      maxZ = Config.CarOutsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Car:client:target:enterLocation',
          label = Lang:t("text.enter_car_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_enter_car_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  local coords = vector3(Config.CarInsideLocation.x, Config.CarInsideLocation.y, Config.CarInsideLocation.z)

  if Config.UseTarget then
    exitZone = exports['qb-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      minZ = Config.CarInsideLocation.z - 1.0,
      maxZ = Config.CarInsideLocation.z + 2.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Car:client:target:exitLocation',
          label = Lang:t("text.exit_car_warehouse"),
        },
      },
      distance = 1.0
    })
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-core']:DrawText(Lang:t("text.point_exit_car_warehouse"), 'left')
      else
        exports['qb-core']:HideText()
      end
...
local function RegisterDutyTarget()
  local coords = vector3(Config.CarDutyLocation.x, Config.CarDutyLocation.y, Config.CarDutyLocation.z)

  if Config.UseTarget then
    dutyZone = exports['qb-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      minZ = Config.CarDutyLocation.z - 2.0,
      maxZ = Config.CarDutyLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Car:client:target:toggleDuty',
...
local function RegisterDeliveyTarget()
  local coords = vector3(Config.CarDropLocation.x, Config.CarDropLocation.y, Config.CarDropLocation.z)

  if Config.UseTarget then
    deliveryZone = exports['qb-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      name = deliveryTargetID,
      heading = 270,
      minZ = Config.CarDropLocation.z - 2.0,
      maxZ = Config.CarDropLocation.z + 1.0,
      debugPoly = false,
    }, {
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Car:client:target:dropPackage',
...
local function GetRandomPackage()
  packageCoords = Config.CarPickupLocations[math.random(1, #Config.CarPickupLocations)]
...
local function SetLocationBlip()
  local RecycleBlip = AddBlipForCoord(Config.CarOutsideLocation.x, Config.CarOutsideLocation.y, Config.CarOutsideLocation.z)
...
local function buildInteriorDesign()
  for _, pickuploc in pairs(Config.CarPickupLocations) do
    local model = GetHashKey(Config.CarWarehouseObjects[math.random(1, #Config.CarWarehouseObjects)])
    RequestModel(model)
...
local function EnterLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.CarInsideLocation.x, Config.CarInsideLocation.y, Config.CarInsideLocation.z)
  buildInteriorDesign()
  DoScreenFadeIn(500)
...
local function ExitLocation()
  ...
  SetEntityCoords(PlayerPedId(), Config.CarOutsideLocation.x, Config.CarOutsideLocation.y, Config.CarOutsideLocation.z + 1)
  DoScreenFadeIn(500)
  ...
  if Config.UseTarget then
    pickupZone = exports['qb-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      ...
      options = {
        {
          type = 'client',
          event = 'qb-recyclejob-Car:client:target:pickupPackage',
...
RegisterNetEvent('qb-recyclejob-Car:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('qb-recyclejob-Car:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('qb-recyclejob-Car:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('qb-recyclejob-Car:client:target:pickupPackage', function()
...
RegisterNetEvent('qb-recyclejob-Car:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('qb-largewarehouse:server:getItemFour')
      GetRandomPackage()
  end)
end)
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          ...
          Wait(500)
          TriggerEvent('qb-recyclejob-Car:client:target:CarenterLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-Car:client:target:CarexitLocation')
          exports['qb-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['qb-core']:KeyPressed()
          Wait(500)
          TriggerEvent('qb-recyclejob-Car:client:target:toggleDuty')
          exports['qb-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-Car:client:target:pickupPackage')
            exports['qb-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['qb-core']:KeyPressed()
            Wait(500)
            TriggerEvent('qb-recyclejob-Car:client:target:dropPackage')
            exports['qb-core']:HideText()
          end
        end
      end
